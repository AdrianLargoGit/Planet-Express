    ...
    public static Fecha fromString(String texto) {
        ...
        if (division.length > 1) {
            ...
            if (tiempo.length == LENGTH) {
                ...
                /*Hemos añadido que lea la hora tanto si solo
                se le pasa la hora como si se le pasa el minuto también para
                que lea correctamente la fecha con hora aun sin haber segundos o minutos.
                */
            } else if (tiempo.length == LENGTH-1){
                hora = Integer.parseInt(tiempo[0]);
                minuto = Integer.parseInt(tiempo[1]);
                segundo = 0;
            } else {
